...
#include <Eigen/Core>
#include "../include/reconstruction/delaunator.hpp"

using std::placeholders::_1;
using std::placeholders::_2;

class ReconstructionDLNNode : public rclcpp::Node {
public:
    // 定义同步策略：近似时间同步
    ...
    ReconstructionDLNNode() : Node("reconstruction_g_dln_ng_obb_node") {
        // 1. 声明并读取参数
        ...
        this->declare_parameter<double>("max_edge_length", 80.0);
        ...
        max_edge_length_ = this->get_parameter("max_edge_length").as_double();
        ...
        sync_->registerCallback(std::bind(&ReconstructionDLNNode::sync_callback, this, _1, _2));
        ...
        RCLCPP_INFO(this->get_logger(), "    - DLN Max edge length: %f", max_edge_length_);
    }
...
private:
        ...
        const pc_msgs::msg::ClusteredPointCloud::ConstSharedPtr& cluster_msg) const
    {
        ...
        if (ground_points_count > 0)
        {
            ...
            try
            {
                // 1.1 提取 XY 坐标进行 2D 投影
                std::vector<double> coords;
                coords.reserve(ground_pcd->points_.size() * 2);
                for (const auto& pt : ground_pcd->points_) {
                    coords.push_back(pt.x());
                    coords.push_back(pt.y());
                }

                // 1.2 执行极速 2D Delaunay 三角剖分
                delaunator::Delaunator d(coords);

                auto terrain_mesh = std::make_shared<open3d::geometry::TriangleMesh>();
                terrain_mesh->vertices_ = ground_pcd->points_;

                // 1.3 过滤多余“裙边” (基于边长阈值剔除凹包外部拉扯的三角形)
                double max_edge_length = max_edge_length_;
                double max_sq_len = max_edge_length * max_edge_length;
                ...
                for (std::size_t i = 0; i < d.triangles.size(); i += 3) {
                    ...
                    const auto& p0 = terrain_mesh->vertices_[idx0];
                    const auto& p1 = terrain_mesh->vertices_[idx1];
                    const auto& p2 = terrain_mesh->vertices_[idx2];

                    // 只要有一条边过长，就认为是边缘多余的连接，直接舍弃
                    if ((p0 - p1).squaredNorm() > max_sq_len ||
                        (p1 - p2).squaredNorm() > max_sq_len ||
                        (p2 - p0).squaredNorm() > max_sq_len) {
                        continue;
                    }

                    // 1.4 统一法线朝向 (通过叉乘判定Z轴方向朝上)
                    Eigen::Vector3d v1 = p1 - p0;
                    Eigen::Vector3d v2 = p2 - p0;
                    if (v1.cross(v2).z() < 0) {
                        terrain_mesh->triangles_.emplace_back(idx0, idx2, idx1);
                    } else {
                        terrain_mesh->triangles_.emplace_back(idx0, idx1, idx2);
                    }
                }
            ...
            {
                RCLCPP_WARN(this->get_logger(), "[!] Ground DLN Reconstruction failed: %s", e.what());
            }
        }
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<ReconstructionDLNNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}